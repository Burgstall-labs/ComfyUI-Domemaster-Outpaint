...
class EquirectToDomemaster:
    ...
    A 1:1 input can represent a 180 x 180 degree square hemisphere. A
    conventional 2:1 full-sphere ERP can either be sampled directly as
    360 x 180 degrees or center-cropped to that same square hemisphere.
    """
        ...
        return {
            ...
            "optional": {
                "input_hfov_deg": ("FLOAT", {
                    ...
                    "tooltip": "Angular width of the input. Use 180 for a 1:1 square "
                    "hemisphere; 360 for a conventional 2:1 full ERP.",
                }),
                "input_vfov_deg": ("FLOAT", {
                    "default": 180.0, "min": 90.0, "max": 180.0, "step": 1.0,
                    "tooltip": "Angular height of the input. A full ERP and square hemisphere "
                    "normally both use 180.",
                }),
                "center_crop_2_1": ("BOOLEAN", {
                    "default": False,
                    "tooltip": "For a 2:1 full ERP, use only its centered 1:1 square as a "
                    "180 x 180 degree hemisphere. This overrides the input spans to 180 / 180.",
                }),
                "batch_size": ("INT", {
    ...
    def render(self, image, size, fov_deg, dome_tilt, yaw_deg, pitch_deg, roll_deg,
               ...
               center_crop_2_1=False, batch_size=8, mirror_x=False):
        device = image.device
        B, H, W, C = image.shape
        ...
        if center_crop_2_1:
            if W < H:
                raise ValueError(
                    "center_crop_2_1 requires an input at least as wide as it is tall; "
                    f"received {W}x{H}"
                )
            source_ratio = W / H
            if abs(source_ratio / 2.0 - 1.0) > 0.05:
                logger.warning(
                    "center_crop_2_1 was enabled for a %dx%d input (aspect %.3f), not a "
                    "conventional 2:1 ERP; the centered %dx%d square will still be used.",
                    W, H, source_ratio, H, H,
                )
            crop_left = (W - H) // 2
            image = image[:, :, crop_left:crop_left + H, :]
            W = H
            input_hfov_deg = 180.0
            input_vfov_deg = 180.0
            logger.info(
                "EquirectToDomemaster: center-cropped input to columns %d:%d (%dx%d), "
                "interpreted as a 180x180 degree hemisphere",
                crop_left, crop_left + W, W, H,
            )
        ...
        if abs(actual_ratio / expected_ratio - 1.0) > 0.05:
            logger.warning(
                ...
                "Use 180x180 for a 1:1 hemisphere, 360x180 for a full 2:1 ERP, or enable "
                "center_crop_2_1 to extract the central hemisphere from a 2:1 ERP.",
        ...
        # Geometry is shared by every frame. Chunking avoids a BxSxSx2 grid
        # copy and converting the entire video to float32 at once.
